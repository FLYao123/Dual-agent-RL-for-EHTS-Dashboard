...
function ratio(numerator, denominator) {
  return denominator > 0 ? clampPercent(100 * numerator / denominator) : 0;
}
...
async function renderDiagnostics(bounds) {
  ...
  const evUtil = clampPercent(mean(s.evUtilColumns.flatMap((name) => slice(name, bounds))));
  const hvUtil = clampPercent(mean(s.hvUtilColumns.flatMap((name) => slice(name, bounds))));
...
function setView(view) {
  ...
  for (const name of ["vehicle", "energy", "balance", "readme"]) {
    document.getElementById(`${name}-view`).classList.toggle("active", view === name);
  }
  document.querySelector(".week-summary").hidden = view === "readme";
